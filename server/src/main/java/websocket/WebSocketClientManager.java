...
public class WebSocketClientManager {
    ...
    public WebSocketClientManager(String serverUrl, ClientMessageObserver observer) {
        ...
        try {
            ...
            System.err.println("URI syntax error when connecting WebSocket: " + e.getMessage());
            if (observer != null) observer.onError(new ServerMessageError("URI error: " + e.getMessage()));
        } catch (IOException e) {
            System.err.println("IO error when connecting WebSocket: " + e.getMessage());
            if (observer != null) observer.onError(new ServerMessageError("IO error: " + e.getMessage()));
        } catch (InterruptedException | ExecutionException | TimeoutException e) {
            System.err.println("Error in WebSocket conection: " + e.getMessage());
            if (observer != null) observer.onError(new ServerMessageError("Error connecting WebSocket server: " + e.getMessage()));
        } catch (Exception e) {
            System.err.println("Unexpected error connecting WebSocket: " + e.getMessage());
            e.printStackTrace();
            if (observer != null) observer.onError(new ServerMessageError("Unexpected error: " + e.getMessage()));
        }
    }
    ...
    public void onMessage(String message) {
        try {
            ...
            e.printStackTrace();
            if (observer != null) {
                observer.onError(new ServerMessageError("Error processing server message: " + e.getMessage()));
            }
        }
    }
