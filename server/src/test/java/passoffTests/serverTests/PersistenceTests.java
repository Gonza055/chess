...
public class PersistenceTests {
    ...
    public void persistenceTest() throws TestException {
        var initRowCount = getDatabaseRows();
        ...
        Assertions.assertTrue(initRowCount < getDatabaseRows(), "No new data added to database");
        stopServer();
        startServer();
    ...
    private int getDatabaseRows() {
        ...
        try {
            ...
            try (Connection conn = (Connection) getConnectionMethod.invoke(obj);) {
                try (var statem = conn.createStatement()) {
                    for (String table : getTables(conn)) {
                        var sql = "SELECT count(*) FROM " + table;
                        try (var resultSet = statem.executeQuery(sql)) {
                            if (resultSet.next()) {
